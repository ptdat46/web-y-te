...
- Đăng ký, xác thực email, đăng nhập, đăng xuất, refresh JWT bằng HttpOnly cookie.
- Quên và đặt lại mật khẩu qua email với mã dùng một lần, hết hạn sau 10 phút.
...
- **Biểu đồ diễn biến sinh hiệu** theo thời gian (nhiệt độ, nhịp tim, huyết áp, SpO2) với vùng ngưỡng an toàn, chọn khoảng 7/30/90 ngày.
- **Xuất bệnh án ra CSV** (lọc theo khoảng thời gian) và **PDF** (từng hồ sơ) để lưu trữ/in ấn.
- **Trang chi tiết bệnh nhân** cho bác sĩ: thông tin, biểu đồ, cảnh báo, bệnh án gần đây.
...
|---|---|---|
| Auth | `/auth/register/`, `/auth/verify-email/`, `/auth/resend-verification/`, `/auth/login/`, `/auth/refresh/`, `/auth/forgot-password/`, `/auth/reset-password/`, `/auth/me/` | Public/authenticated tùy endpoint |
| Catalog | `/catalog/diseases/`, `/catalog/symptoms/` | Public |
| Doctors | `/doctors/`, `/doctors/me/` | Public hoặc doctor/admin |
| Connections | `/connections/`, `/connections/{id}/respond/` | Patient gửi request/hủy; doctor duyệt hoặc từ chối |
| Care | `/records/`, `/records/export/csv/`, `/records/{id}/pdf/`, `/vitals/` (hỗ trợ `?from=&to=&limit=&patient_id=`), `/alerts/`, `/notifications/` (kèm `read/`, `read-all/`) | Patient/doctor theo object permission |
| Patients (bác sĩ) | `/patients/`, `/patients/{id}/` | DOCTOR có kết nối APPROVED |
...
| `VitalsPage.tsx` | Xem/thêm sinh hiệu, **biểu đồ diễn biến** với bộ chọn khoảng thời gian |
| `RecordsPage.tsx` | Xem/thêm/cập nhật bệnh án, **xuất CSV** theo khoảng, **PDF** từng hồ sơ |
| `AlertsPage.tsx` | Xem và cập nhật trạng thái cảnh báo |
| `DoctorsPage.tsx` | Bệnh nhân tìm bác sĩ |
| `ConnectionsPage.tsx` | Bác sĩ xem kết nối được duyệt, có nút **Xem hồ sơ** bệnh nhân |
| `PatientsPage.tsx` | Bác sĩ xem **danh sách bệnh nhân** kết nối và thống kê nhanh |
| `PatientDetailPage.tsx` | Bác sĩ xem **hồ sơ chi tiết** bệnh nhân: thông tin, biểu đồ, cảnh báo, bệnh án |
...
| `ChangePasswordPage.tsx` | Đổi mật khẩu bắt buộc (áp dụng cho mọi role) |
| `components/VitalsChart.tsx` | Biểu đồ sinh hiệu dùng recharts (5 series, vùng ngưỡng an toàn) |
